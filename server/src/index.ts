...
// NOTE: Plugin registration (cors, fastifyStatic) is done inside startServer()
// so that no top-level await exists at module scope — required for CJS bundling.
...
export async function startServer(overridePort?: number) {
  ...
  const isTauriSidecar = process.env.TAURI_SIDECAR === 'true';
  ...
  // Register plugins (must happen before listen; no top-level await allowed in CJS)
  await app.register(cors, { origin: true });

  // Serve static files only in standalone mode.
  // In Tauri sidecar mode __dirname resolves to the SEA exe's directory,
  // so '../public' doesn't exist; Tauri's webview serves the frontend directly.
  if (!isTauriSidecar) {
  ...
  logger.info({ port: actualPort, host: "0.0.0.0", address }, `Server listening on ${address}`);
...
// When run as a Node.js SEA the ESM main-module guard
// (process.argv[1] === fileURLToPath(import.meta.url)) never matches,
// so we call startServer() unconditionally here.
