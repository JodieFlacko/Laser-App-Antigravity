...
async function verifyLightBurnFile(filePath: string, orderId: string): Promise<void> {
  ...
  try {
    ...
    const verificationError = new Error(
      `LIGHTBURN_FILE_VERIFICATION_FAILED: Failed to verify generated file at ${filePath}. ${error instanceof Error ? error.message : "File may not exist"
      }`
    );
...
export async function generateLightBurnProject(
  ...
  try {
    ...
    throw new Error(
      `Failed to generate LightBurn project: ${error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
}
