...
export async function syncOrders(): Promise<SyncResult> {
  ...
  // Guard: feed URL must be configured before sync can run.
  // An empty string would cause fs.createReadStream('') to resolve to the
  // current working directory (a directory), throwing EISDIR.
  if (!feedUrl || feedUrl.trim() === '') {
    throw new Error(
      'Feed URL not configured. Please open Settings and set the feed URL before syncing.'
    );
  }
