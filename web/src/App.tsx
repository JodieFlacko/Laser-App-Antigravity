...
export default function App() {
  ...
  // Resolved API base URL — empty until getApiBaseUrl() resolves on mount
  ...
  // Resolve API base URL once on mount (handles Tauri sidecar port handoff)
  useEffect(() => {
    getApiBaseUrl().then(setApiUrl);
  }, []);


  // In Tauri, process lifetime is owned by Rust (SIGTERM on window close).
  // The heartbeat / beacon shutdown system is only needed in browser mode.
  const isTauri = typeof window !== 'undefined' && !!(window as any).__TAURI_INTERNALS__;

  // ==================== GRACEFUL SHUTDOWN: HEARTBEAT SYSTEM ====================
  // Send periodic heartbeat to server to indicate this client is alive
  useEffect(() => {
    if (!apiUrl || isTauri) return; // skip in Tauri — Rust owns process lifetime
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      try {
        ...
        navigator.sendBeacon(`${apiUrl}/api/shutdown`, blob);
      } catch {
        // Fallback for browsers that don't support sendBeacon (rare)
        fetch(`${apiUrl}/api/shutdown`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: payload,
  ...
  const fetchOrders = async (term: string, mode: 'pending' | 'all', silent = false) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/orders?limit=10000&offset=0${searchParam}${statusParam}${customFieldParam}`
      );
  ...
  const handleSideProcessing = async (orderItemId: string, side: 'front' | 'retro') => {
    ...
    try {
      const response = await fetch(`${apiUrl}/orders/item/${orderItemId}/lightburn/${side}`, {
        method: "POST"
      });
      const data = await response.json();
  ...
  const handleLightburn = async (orderId: string) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/orders/${orderId}/lightburn`, {
        method: "POST"
      });
      const data = await response.json();
  ...
  const handleDiscardReprint = async (orderId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/orders/${orderId}/discard-reprint`, {
        method: "POST"
      });
  ...
  }, [apiUrl, debouncedSearchTerm, filterMode]);
  ...
  useEffect(() => {
    if (!apiUrl || !hasPendingHydration) return;
    const timer = setInterval(() => {
      fetchOrders(searchTerm, filterMode, true);
    }, 3000);
    return () => clearInterval(timer);
  }, [apiUrl, hasPendingHydration, searchTerm, filterMode]);
  ...
  // ==================== RENDER ====================
  // Don't render anything until the API URL is known
  ...
  const wasFullyPrintedBefore = (order: Order) => {
    ...
    const frontDone =
      !hasFrontCustomData || order.frontePrintCount >= order.quantity;
    const retroDone =
      !hasRetroCustomData ||
      (order.retroStatus === 'printed' && order.retroPrintCount >= order.quantity) ||
      (order.retroStatus === 'processing' && order.retroPrintCount >= order.quantity) ||
  ...
  if (viewState.view === "settings") {
    return <Settings onBack={() => setViewState({ view: "orders" })} suggestedSku={viewState.suggestedSku} apiUrl={apiUrl} />;
  }
  ...
  return (
    <div className="min-h-screen px-6 py-10">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 text-sm font-medium text-slate-700">
            <div className="flex items-center gap-2">
              <button
                className={`rounded px-3 py-1.5 text-sm font-medium transition-colors ${filterMode === 'pending'
              ...
              <button
                className={`rounded px-3 py-1.5 text-sm font-medium transition-colors ${filterMode === 'all'
                  ? 'bg-indigo-100 text-indigo-700'
                  : 'text-slate-600 hover:bg-slate-100'
                  }`}
      ...
      {/* Fixed toast — renders outside the flow so it never shifts the layout */}
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-start gap-3 rounded-lg border px-4 py-3 shadow-lg text-sm max-w-sm animate-slideInRight ${toast.type === 'error'
        ...
        >
          ...
          <button
            ...
            className="ml-auto -mr-1 opacity-50 hover:opacity-100 transition-opacity"
            aria-label="Dismiss"
          >
            <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
